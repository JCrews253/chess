...
const Board = () => {
    ...
    const CheckForCheck = (idx:number) => {
    ...
    const ShowPawnMoves = (idx:number) => {
        ...
        if(board[idx].color === Color.light){
            ...
            if(isFirst && board[GetIdxByRowCol(row-1,col)].piece === Piece.empty && board[GetIdxByRowCol(row-2,col)].piece === Piece.empty) moves.push(GetIdxByRowCol(row-2,col))
